...
function review_shortcode($atts, $content) {
	...
	if ($cat) {
     $query = array(
        ...
        $wp_query_review = new WP_Query($query);
    }
    $items_src .= ' 
   <div class="home-shr clearfix">
	  <div class="adshr-col">
		 <div class="home-width">';
    while ($wp_query_review->have_posts()):
        $wp_query_review->the_post();
        global $post;
        $fix         = the_excerpt_max(0);
        $custom      = get_post_custom($post->ID);
        $title       = get_the_title();
        $image_id    = get_post_thumbnail_id();
        $cover       = wp_get_attachment_image_src($image_id, 'review-shortcode');
        $cover_large = wp_get_attachment_image_src($image_id, 'photo-large');
        $genre       = $custom["review_genre"][0];
